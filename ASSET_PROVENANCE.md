...
| --- | --- | --- | --- | --- | --- | --- |
| `public/images/portrait-{480,960}.{avif,webp}`, `portrait-640.jpg` | Photographer credited in the file's EXIF as "Leo" (Canon EOS RP, 30 May 2025) | Supplied by Alexandros Kaminas for this site | **Not yet documented** — copyright rests with the photographer unless assigned | None displayed; the privacy page states the photo remains the photographer's copyright | Cropped to 4:5, resized, re-encoded; all EXIF/IPTC metadata (including the artist name, camera and timestamps) stripped by `scripts/make-assets.mjs` | **Owner to confirm** that the photographer permits use on this website (a commissioned portrait can normally be reproduced by the person portrayed under Dutch Auteurswet art. 19, but a written OK is safer). Only the owner is depicted, so no third-party personality rights are involved. |
