...
- No cookies, `localStorage`, `sessionStorage` or IndexedDB used by the site's own code. The colour scheme follows `prefers-color-scheme` and is not stored.
...
- This site stores and reads nothing on the device. Nothing falls within art. 11.7a; there is nothing to consent to.
- **Decision: no cookie banner and no separate cookie policy.** A short, accurate "Cookies and local storage" section is included in the privacy notice instead. This is the builder's reading of the rule (the regulators' pages describe when consent *is* needed rather than stating the zero-cookie case explicitly), and it is the conventional one.
...
Any of the following changes the analysis and must be handled *before* deploying: adding analytics or any third-party script/embed; adding a contact form or newsletter; adding a theme toggle or any use of local storage; adding cookies of any kind (then: cookie policy, consent before loading, equal accept/reject, withdrawal mechanism, documented categories/providers/lifetimes); reproducing publisher figures or logos; publishing photographs of other people; offering paid services; moving to a different host or domain.
